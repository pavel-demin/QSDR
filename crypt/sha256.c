...
#define LOAD32H(x, y)  \
   { ((unsigned char*)(&x))[0] = ((unsigned char*)(y))[3]; \
     ...
     ((unsigned char*)(&x))[3] = ((unsigned char*)(y))[0]; } \
...
void sha256_done(sha256_state * md, uint8 *out)
{
    ...
    /* if the length is currently above 56 uint8s we append zeros
